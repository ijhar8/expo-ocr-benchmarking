...
import { recognizeWithMLKit } from '../../src/engines/mlkit';
...
export default function MLKitScreen() {
  ...
  const handleCapturePhoto = async () => {
    ...
    try {
      ...
      const result = await recognizeWithMLKit({ uri, source: 'camera_capture' });
  ...
  const handlePickImage = async () => {
    try {
      ...
      if (!pickRes.canceled && pickRes.assets && pickRes.assets[0]) {
        ...
        const result = await recognizeWithMLKit({ uri, source: 'gallery' });
        setLastResult(result);
        setActiveOCRResult(result);
        setIsSaved(false);
      }
  ...
  const handleTestSample = (sample: SampleInvoice) => {
    ...
    const latency = Math.floor(Math.random() * 40) + 45; // ~45-85ms simulated native ML Kit execution
    ...
    setTimeout(() => {
      const result: OCRBenchmarkResult = {
        id: `mlkit_sample_${Date.now()}`,
        engine: 'mlkit',
        timestamp: Date.now(),
        source: 'sample',
        fullText: sample.mockOcrText,
        lines: lines.map(l => ({ text: l, confidence: 0.96 })),
  ...
  const handlePdfPage = async (uri: string, pageIndex: number, totalPages: number) => {
    ...
    try {
      const result = await recognizeWithMLKit({ uri, source: 'gallery' });
      // annotate result with pdf page info
      const annotatedResult = { ...result, id: `mlkit_pdf_p${pageIndex + 1}of${totalPages}_${Date.now()}` };
      setLastResult(annotatedResult);
      setActiveOCRResult(annotatedResult);
      setIsSaved(false);
    } catch (err: any) {
      Alert.alert('PDF OCR Error', err?.message || 'Failed to process PDF page');
    } finally {
      setIsProcessing(false);
    }
  };
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      <View style={styles.topBanner}>
        <View style={styles.bannerBadge}>
          <Text style={styles.bannerBadgeText}>TIER 1 BASELINE</Text>
        </View>
        <Text style={styles.bannerTitle}>ML Kit / Apple Vision (Native)</Text>
        <Text style={styles.bannerDesc}>
          Zero binary overhead • Native hardware acceleration • Devanagari & Latin script support
        </Text>
      </View>
      ...
      <View style={styles.viewportCard}>
        ...
        {isProcessing && (
          <View style={styles.loadingOverlay}>
            ...
            <Text style={styles.loadingText}>Running ML Kit OCR...</Text>
          </View>
        )}
      </View>
      ...
      <View style={styles.actionsRow}>
        ...
        >
          ...
          <Text style={[styles.secondaryActionText, { color: '#38BDF8' }]}>Samples</Text>
        </TouchableOpacity>
      </View>
